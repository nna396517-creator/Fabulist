...
`npm run demo` 全自動，不需要任何點擊：它會在 `http://localhost:8787/chat.html` 提供示範聊天室、用一個獨立的暫存設定檔（`.chrome-profile/`，已加入 `.gitignore`）啟動 Chrome、透過 DevTools 協定的 `Extensions.loadUnpacked` 載入 `dist/`、幫你把 `localhost` 加進啟用清單、重新載入示範頁，最後送出循環播放的 Demo。HUD 會每兩秒切換一次，依序播放開心、困惑、生氣、感謝並不斷重複，讓你看完血條升降、變化量浮出、頭像動畫、狀態徽章與戰鬥紀錄。之所以不用 `--load-extension` 參數，是因為新版 Chrome 穩定版已經不理會它。

想走完整流程（會真的呼叫 API）：點擴充套件圖示填好 API key，再按示範頁上的「收到一則新訊息（對方）」，HUD 會進入分析中的閃爍狀態，回來之後就能用 Tab 採用建議。popup 裡的「Demo 展示」按鈕則是播一輪就停。
