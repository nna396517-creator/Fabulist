...
HUD 只在你啟用的網站出現，而且會盡量停在訊息區外面：面板優先停靠在聊天欄位「右側」的空白處，與輸入框底部對齊；右邊放不下就改停左側；兩邊都擠不下時才退回輸入框正上方，並且自動收成只剩血條的樣子，把擋住的面積壓到最小。面板寬 360 像素，捲動與改變視窗大小都會重新定位，離視窗邊緣永遠留 12 像素。
...
| 收合鈕 | 右上角 `[-]`，收合後只留頭像與血條，選擇會記在 `chrome.storage.local`。你自己按過就以你的選擇為準，沒按過時由版面決定（停靠得下就展開，擠不下就收合） |
...
`npm run demo` 全自動，不需要任何點擊：它會在 `http://localhost:8787/chat.html` 提供示範聊天室、用一個獨立的暫存設定檔（`.chrome-profile/`，已加入 `.gitignore`）啟動 Chrome、透過 DevTools 協定的 `Extensions.loadUnpacked` 載入 `dist/`、幫你把 `localhost` 加進啟用清單、重新載入示範頁，最後送出循環播放的 Demo。之所以不用 `--load-extension` 參數，是因為新版 Chrome 穩定版已經不理會它。

播的不是四張靜態畫面，而是一段會一直跑下去的對話，每一步都走跟真實流程一樣的程式路徑：

1. 對方（小林）送出一則訊息，訊息區左側浮出灰色泡泡（300 毫秒淡入）
2. 隔 600 毫秒，HUD 進入分析中：頭像變 🤔、血條閃爍，持續 1.1 秒
3. 判讀結果上場：血條升降、變化量浮出、頭像動畫、狀態徽章、一句話摘要與三張建議卡，輸入框裡同時出現灰色的建議文字
4. 停 1.6 秒讓你看清楚建議，接著走跟真的按 Tab 完全相同的路徑採用它，戰鬥紀錄印出 `> 已按 Tab 採用建議`
5. 再隔 900 毫秒，回覆以藍色泡泡送出、輸入框清空，紀錄印出 `> 已送出回覆`
6. 隔 1.4 秒換下一則訊息；四步跑完會先把聊天室清回最初的兩則訊息再從頭來過，訊息清單不會無限長大

四個步驟依序是困惑、生氣、感謝、開心，串起來是一段「客人搞不懂流程、等到不耐煩、問題解決回來道謝、收到貨很滿意」的小劇本。內容腳本只透過 `document` 上的 `cma:demo` 事件（`incoming` / `reply` / `reset`）跟示範頁溝通，不會直接去動網頁自己的 DOM。

想走完整流程（會真的呼叫 API）：點擴充套件圖示填好 API key，再按示範頁上的「收到一則新訊息（對方）」，HUD 會進入分析中的閃爍狀態，回來之後就能用 Tab 採用建議。popup 裡的「Demo 展示」按鈕則是同一套編排播一輪就停，不會清空聊天室。
...
- HUD 用 `position: fixed` 停在聊天欄位旁邊；視窗很窄、左右都塞不下 360 像素的面板時，才會退回輸入框上方並自動收合，這種情況還是會擋到一點訊息
- 結果只存在記憶體，重新整理分頁就沒了
